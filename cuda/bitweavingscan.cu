...
__global__ void static calc_early_bit(int * early_bit, int *eq, int n, int early_size){
    int offset = blockIdx.x*blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    if(offset % early_size  != 0 ) return;
    for(int i=offset; i<n; i+=stride){
    	early_bit[i / early_size] = 0;
    	for(int j = i; j < i + early_size ; j ++ )    
        	early_bit[i / early_size] |= (eq[j]);
    	if(early_bit[i / early_size] == 0 )early_bit[i / early_size] = 1;
    	else early_bit[i / early_size] = 0;
    }
                   ...
                   unsigned int loopTotal)
{
  ...
  for(int loop = 1; loop <= loopTotal; loop++){  
      ...
      equal<<<grid,block>>>(lt, n/32, 0) ;
      equal<<<grid,block>>>(eq, n/32, c) ;
	  equal<<<grid,block>>>(early_bit_gpu, n / early_size , 0);
      // printf("%d\n",    clock());
      for(int j = 0; j < 32; ++j){
      	    int constC = 0;
            for(int k = 0;k < 32;++k) {
            ...
            calc_early_bit<<<grid,block>>>(early_bit_gpu, d + j * (n / 32), n / 32, early_size / 32);
           	checkCuda(cudaThreadSynchronize());
	        checkCuda( cudaMemcpy(early_bit_cpu, early_bit_gpu, n / early_size, cudaMemcpyDeviceToHost));
      }
...
int main(int argc, char ** argv)
{
  ...
  // host arrays
  int *h_aPageable, *h_bPageable,*h_bitPageable,*early_bit_cpu;   
  int *h_aPinned, *h_bPinned;

  // device array
  int *d_a,*lt,*eq,*early_bit_gpu;

  // allocate and initialize
  h_aPageable = (int*)malloc(bytes );          
  h_bPageable = (int*)malloc(bytes );
  h_bitPageable =(int *)malloc(bytes );
  early_bit_cpu = (int *)malloc(bytes );             // host pageable
  ...
  checkCuda( cudaMalloc((void**)&early_bit_gpu, bytes ) ); 
  int early_size = 32; 
 ...
 // profilescan(h_aPageable, h_bPageable, d_a, filter, nElements, constC,"Pageable",20);
  ...
  profilebitweavscan(h_bitPageable, h_bPageable, d_a, lt, eq, early_bit_cpu, early_bit_gpu, early_size, nElements, constC,"Pageable",1);
