  ...
  for(int loop = 1; loop <= loopTotal; loop++){  
      ...
      double len_E=0.0;
        for(int k  = 0; k < n ; k += early_size ){
            ...
            // for(int j = 0; j < len; ++j){     
            //     for(int i = 0; i < early_size / group_size; i ++ ) 
            //       constC[j * early_size / group_size + i] = bit_constC(know_stop_constC_cpu[k / group_size + i], j);
            // }
            // checkCuda( cudaMemcpy(gpu_constC, constC, early_size / group_size *4 * len,  cudaMemcpyHostToDevice) );      

            // for(int j = 0; j < len; ++j){              
            //     int place = k + early_size / 32 * j;
            //     genScanFilter_int_lth_bit<<<grid,block>>>(d + place, early_size / 32,  gpu_constC + j * early_size / group_size, group_size/32, lt + k/32, eq + k/32);
            //     checkCuda(cudaThreadSynchronize());
            // }
            for(int j = 0; j < len; ++j){     
                for(int i = 0; i < early_size / group_size; i ++ ) 
...
int main(int argc, char ** argv)
{
 ...
 // profilescan(h_aPageable, h_bPageable, d_a, filter, nElements, constC,"Pageable",20);
  ...
  profilebitweavscan(h_bitPageable, h_bPageable, d_a, lt, eq, know_stop_len_cpu, know_stop_constC_cpu,early_size, group_size, nElements,gpu_constC,cpu_constC,"Pageable",1);
  // printf("constC=%d\n",constC);
